...
def setup_dbt_project(project_dir: str):
    ...
    _write_if_missing(root / "models" / "staging" / "stg_chevron_table.sql", STG_CHEVRON_SQL)
    _write_if_missing(root / "models" / "staging" / "stg_fuel_consumption.sql", STG_FUEL_ECONOMY_SQL)
    _write_if_missing(root / "models" / "staging" / "stg_vehicle_sales.sql", STG_VEHICLE_SALES_SQL)    
